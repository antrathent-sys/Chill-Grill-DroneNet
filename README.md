...
| `lib/hailmap.lua` | What the pocket shows while a taxi comes. `gauge` is the default and is pure text, as a terminal would print it: rules of equals signs, UNIT / STATUS / RANGE / ETA in a label column, a `[####....]` loading bar with a percentage, a running log of the job, and a blinking prompt at the foot. `M` swaps to the map: the customer fixed in the middle as a hollow ring, the taxi a solid block with the ground it has covered behind it, rings on round numbers that follow the distance, and the blocks counting down. Both are pure drawing onto a `lib/display.lua` canvas, so `python tools/preview_pocket.py [--screen map] [--theme silo]` renders the very same screens on the desktop. |
