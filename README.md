...
**A dock bridges the networks, and on this pad that is the ONLY way to know.** Confirmed on the ground on 2026-09-11, physically docked: `getConnectedName()` returns `""`, exactly as it does when undocked. It is the connector's entire API, so there is nothing else to ask it. But latching joins the pad's wired network to the craft's and the peripheral count jumps - 32 while docked on this airframe. `fly` records the count at startup, before the connector is ever extended, treats a rise of `DOCK_BRIDGE_MIN` as a dock in its own right, logs it as `npers`, and prints which signal ended the flight. Harness case `dock to an unnamed pad` reproduces it: the mock reports `""` throughout and bridges four peripherals in on latching, and the flight still has to end.

This is also, almost certainly, why a `preflight` once reported 8 thrusters and 8 accumulators on an airframe carrying four of each - it was docked at the time.
