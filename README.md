...
| `lib/hailmap.lua` | The ride screen on the pocket: a boxed terminal dashboard. A title rule, a UNIT panel with the unit and the status, a RANGE panel with the blocks left, a percentage, a filled bar and the ETA, and a LOG panel the job prints itself into. Characters and cell colours only - the bar is filled cells, not hashes - sized for a 26x20 pocket screen, in the theme's own palette. `python tools/preview_pocket.py` draws the same code on the desktop. |
