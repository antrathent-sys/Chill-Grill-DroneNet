...
| `hail.lua` | The customer's terminal, in their pocket. `hail` finds them by GPS and offers a menu of places with the distance to each, the pads `ops` knows plus anything in `places.lua` on that computer (a pads-style table, or plain `name x z` lines); typing coordinates always works too. While the taxi comes it shows which unit is flying, how far away it is, live from `ops`, and a turning slash so a slow answer never looks like a dead screen. `hail 1200 340` is one ride, `hail stats` is that terminal's use, `hail test` asks the base to answer. Amber on black, cut to a 26-column pocket screen. Counts rides, asks, failures and blocks in `.hailstats` and reports to `ops`. Holds no keys. Runs from `startup autorun hail`. |
