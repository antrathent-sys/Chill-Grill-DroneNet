...
**`DOCK_GAP` is measured, not assumed - and measured LATCHED.** Docked, the altimeter reads 5.5 blocks above the pad: four flights began that way at 68.5 over a pad at 63. Sitting on the extended connector *without* latching it reads 70.5, and that was the value in use for a day, so the descent aimed for the unlatched height and left the last two blocks to the magnet. Before that it was 3, and the profile arrived still asking for 7 b/s. To re-measure on a new airframe: dock, take the altimeter reading, subtract the pad Y. The mock's `PARK` has to agree with it, or the harness stops testing the real approach.
