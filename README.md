...
- **continuous approach (2026-09-10)**: for `go`/`dock` there is effectively no brake phase any more. Allowed speed at distance d is √(2·`CRUISE_DECEL`·d); the along-track lean is `CKV·(allowed − actual)`, positive to accelerate and negative up to `BRAKE_DEG` to decelerate, so lean tapers with distance by construction; hold takes over inside `APPROACH_HOLD_DIST` below `APPROACH_HOLD_SPD`. The brake phase remains as a backstop (`BRAKE_K` 0.12).
