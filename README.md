...
| `lib/hailmap.lua` | What the pocket shows while a taxi comes. `gauge` is the default: how far away in big digits, a bar that fills as it closes, and the time left worked out from how fast the gap is shrinking. `M` swaps to the map: the customer fixed in the middle as a hollow ring, the taxi a solid block with the ground it has covered behind it, rings on round numbers that follow the distance, and the blocks counting down. Both are pure drawing onto a `lib/display.lua` canvas, so `python tools/preview_pocket.py [--screen map] [--theme silo]` renders the very same screens on the desktop. |
