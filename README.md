...
| `lib/hailmap.lua` | What the pocket shows while a taxi comes. `gauge` is the default, an imperial terminal readout: the cog badge and the unit up top, the status in capitals, the range in big digits, a segmented VECTOR bar, and an ETA line, with a blinking cursor in the corner. `M` swaps to the map: the customer fixed in the middle as a hollow ring, the taxi a solid block with the ground it has covered behind it, rings on round numbers that follow the distance, and the blocks counting down. Both are pure drawing onto a `lib/display.lua` canvas, so `python tools/preview_pocket.py [--screen map] [--theme silo]` renders the very same screens on the desktop. |
