...
| `hail.lua` | Call a taxi from wherever you are standing, on a wireless pocket computer. `hail` finds the customer with GPS and asks where to; `hail 1200 340` skips ahead. It reaches `ops` by radio, which is a request and never an order: `ops` decides, rate-limits each caller and sends the flight command to the drone over the wire, so no drone ever takes a flight command from the air. |
