...
| `hail.lua` | The customer's terminal, in their pocket. `hail` finds them by GPS, asks where to, and calls a drone that lands beside them; `hail 1200 340` is one ride; `hail stats` is what that terminal has been used for, and `hail test` asks the base to answer so you can tell out-of-range from no-drone-free. It counts its own rides, asks, failures and blocks in `.hailstats` and reports to `ops` after each one. It reaches `ops` by radio, which is a request and never an order: `ops` decides, rate-limits each caller, and the flight command it sends is sealed with the drone's key, so nothing flies on an unauthenticated message. Holds no keys. Runs from `startup autorun hail`. |
...
python tools/run_fleet_test.py         lib/fleet.lua, 71 cases
python tools/run_beacon_test.py        beacon.lua incl. taking orders, 39 cases
```
