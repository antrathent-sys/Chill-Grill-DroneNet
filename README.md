...
| `lib/fleet.lua` | The language the fleet takes orders in: taxi requests, job assignments, acks, job states and pad usage counters, plus who is free to take a job and which `fly` command each leg of a job becomes. Wired rednet only, protocol `dronenet`. Pure; tested by `tools/run_fleet_test.py`. |
| `ops.lua` | The admin panel on the base. `ops` is the live board and dispatches taxi pads, `ops list` is one snapshot, `ops fly <drone|any> <command>` flies any drone that is docked and free, `ops send <who> <pad>` is shorthand for ferrying one to a pad, `ops land|hold|undock` sends fly's own in-flight words, `ops stats` collects what the pads report. Telemetry comes in sealed on the ender modem and is never answered there; every order leaves on the wired network. |
...
python tools/run_fleet_test.py         lib/fleet.lua, 59 cases
python tools/run_beacon_test.py        beacon.lua incl. taking orders, 36 cases
```
