...
**A dock bridges the networks, and that is a second way to know.** `getConnectedName()` is the connector's whole API, and an unlabelled pad may report an empty string while physically latched - which would look exactly like not being docked. But docking joins the pad's wired network to the craft's, so the number of visible peripherals jumps. `fly` records that count as `npers`, treats an increase as a dock in its own right, and says which signal ended the flight. It is very likely the explanation for a `preflight` once reporting 8 thrusters and 8 accumulators on an airframe carrying four of each. `docktest` now prints the raw return value and the peripheral count too.
