...
| `lib/pads.lua` | The named places, in `pads.lua` on the computer (never in the repo - places are per world). There are two kinds. A **dock** is a built facility: the connector latches, the craft charges, cargo can be loaded. A **pad** is just a known-good place to land. A craft *ferries* to a dock and *lands* at a pad, and `fly dock`/`fly ferry` refuse a pad rather than try to latch onto a field. `fly pads` lists them, `fly pad add <name> [pad]` records one from where the craft is standing (a dock unless told otherwise), `ops place add <name> <x> <z> [y] [dock]` types one in from the map (a pad unless told otherwise), `fly land <name>` lands at either kind. A record from before the kind existed is a dock. A place called `home` replaces `CFG.HOME_X/Y/Z`. Tested by `tools/run_pads_test.py`. |
