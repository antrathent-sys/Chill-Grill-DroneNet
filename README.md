...
**Altitude.** `HOVER + integ + AKD·(vWant − v)` with `vWant = (AKP/AKD)·e` capped at `CLIMB_RATE`: a rate cascade. Vertical speed comes from Sable's linear velocity, not differenced altitude (which skipped ticks and slammed the throttle on/off). The rate request ramps at `VRATE_SLEW` b/s² (a step to full rate rang 13→8→11 b/s for ten seconds) and the integrator only runs inside `INTEG_BAND` of hover, so a 400-block climb no longer arrives with a wound-up integrator and a +5 overshoot. `CLIMB_RATE` 20: 10 b/s cost 0.26–0.32 power, barely above hover.
