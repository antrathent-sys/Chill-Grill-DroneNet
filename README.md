...
| `lib/ledger.lua` | Money: an append-only `ledger.csv` of credits and fares, balances that are simply their sum, the tariff (a rate per block, a minimum that covers the climb and descent, and the places that are free to travel to) and spurs formatted the way people say them. Balances may go negative on purpose, and a ride home is always free, so an empty account cannot strand anyone. A half-written last line is skipped rather than read as a smaller amount. Pure; tested by `tools/run_ledger_test.py` (27 cases). Copy `tariff.example.lua` to `tariff.lua` on the base to change the rates and to place the pay pad. |
