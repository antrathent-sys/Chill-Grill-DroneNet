...
| `lib/hailmap.lua` | The map the pocket draws while a taxi comes: the customer fixed in the middle as a hollow ring, the taxi a solid block with the ground it has covered behind it, rings on round numbers that follow the distance, and the blocks counting down. Pure drawing onto a `lib/display.lua` canvas, so `python tools/preview_pocket.py` renders the very same screen on the desktop. |
