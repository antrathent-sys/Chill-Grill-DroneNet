...
| `lib/hailui.lua` | The ride screen on the pocket, drawn as a terminal dashboard: thin boxes with their titles set into the top rule, UNIT with the status under it, ETA beside it, RANGE with a filled bar and a percentage, and a LOG the job prints itself into, with the keys on the last line. ComputerCraft has no box-drawing characters, so the boxes are one-pixel lines through `lib/display.lua`'s canvas and the titles are text written into the cells the line passes through. Standard palette: cyan furniture, white values, green progress, red for trouble. `python tools/preview_pocket.py` draws the same code on the desktop. |
