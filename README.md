...
**`DOCK_GAP` is measured, not assumed.** The craft comes to rest with its altimeter reading 7.5 blocks above the pad - the sensor sits 2.8 up the airframe and the legs hold the rest. It was set to 3, so the descent profile aimed 4.5 blocks below anything reachable and was still asking for 7 b/s when it arrived, hitting the pad at about 10. To re-measure on a new airframe: land on the pad, take the altimeter reading where it stops, subtract the pad Y. The mock's pad geometry has to agree with it, or the harness stops testing the real approach.
