...
**Attitude error at high lean.** Above ~75° of true lean both down-vectors are nearly horizontal and the rotation between them lies along the thrust axis, so a pure cross-product error reports pitch/roll error as yaw and the loops go blind (roll ran 60° off at 80° with a 0.3 vector command). The lean *magnitude* error is therefore taken exactly along the body direction of the lean, only the direction part comes from the cross product, and the command cap is pulled down whenever measured true lean exceeds `CRUISE_DEG` (the aero bias adds 5–9° at speed). Physically the thrust vector cannot be steered sideways at 80° without yawing - that is the yaw loop's job.

**Heading rate at lean (open).** The yaw rate fed to the heading filter and damper is the plain world-vertical angular rate - the configuration of the 119 and 136 b/s flights. At 70° of lean that is mostly body roll, and it swung the heading estimate ±25° at 117 b/s; a projection onto the thrust axis was tried, but the sign of its horizontal part could not be pinned down offline and an online sign selector flipped mid-spin and drove two spins. Raw ω is now logged (`wx,wyr,wz`) so the convention can be settled from data before it is tried again.
...
- **continuous approach (2026-09-10)**: for `go`/`dock` there is effectively no brake phase any more. Allowed speed at distance d is √(2·`CRUISE_DECEL`·d); accelerating is the cruise loop's job, along a target curve at half the planned deceleration that ends at the hold radius (so it never re-accelerates into the braking zone); decelerating is kinematic - required decel v²/2d, braking lean atan(decel/`GRAV`) applied against the velocity up to `BRAKE_DEG` - so lean tapers with distance by construction (a proportional version with `CKV` 1 only asked 20° for 20 b/s of overspeed and arrived at 95 b/s); hold takes over inside `APPROACH_HOLD_DIST` below `APPROACH_HOLD_SPD`. The brake phase remains as a backstop (`BRAKE_K` 0.12).
