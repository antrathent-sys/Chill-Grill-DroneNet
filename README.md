...
**Disk space.** A flight writes about 230 bytes a row at 10 Hz, so three minutes is a ~400KB `flightlog`, and the repo itself is another 300KB. On a computer whose space limit has been turned down that runs out during an update - and the failure is `Out of space` on an anonymous line, with the tree left half old and half new. `startup` now reports free space, deletes the disposable files (`flightlog`, `probe.txt`, `preflight.txt`, `probelog.csv`, `stickers.txt` - all either already in the repo or regenerable; never `mixmap.csv`, which is calibration) when it is short, removes each file before rewriting it rather than writing over it, and names the file and the shortfall if a write still fails. `preflight` reports free space and the largest files, and fails outright below 150KB.
