...
| `beacon.lua` | The drone's idle telemetry: while `fly` is not running it sends the same sealed status packet every 2 s (position, height, battery, thruster FE, docked or idle) plus home for the map, so the base sees a drone whenever its computer is on. F asks for a fly command and runs it (the beacon is quiet during the flight), Q stops. Never flies by itself; `startup autorun beacon` runs it from boot. Tested by `tools/run_beacon_test.py`. A pickup at open ground that comes to rest more than 4 blocks above the customer's own ground has landed on something (a roof, a tree): the job is called off as LANDING ZONE OBSTRUCTED and it goes home. A flight it was ordered to fly that fails puts it in DISTRESS: it sends its position sealed, repeats it every 30 s, reads `sos` on every telemetry packet, refuses new customers and does not try to fly home by itself; the next order from the base clears it. |
