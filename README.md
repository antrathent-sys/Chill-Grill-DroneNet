...
| `lib/pads.lua` | The named dock points: the home pad and every depot, in `pads.lua` on the computer (never in the repo - pads are per world). `fly pads` lists them, `fly pad add <name>` records one from where the craft is standing, `fly dock <pad>` and `fly ferry <pad>` fly to one. A pad called `home` replaces `CFG.HOME_X/Y/Z`. Tested by `tools/run_pads_test.py`. |
