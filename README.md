...
**Altitude.** `HOVER + integ + AKD·(vWant − v)` with `vWant = (AKP/AKD)·e` capped at `CLIMB_RATE`: a rate cascade. Vertical speed comes from Sable's linear velocity, not differenced altitude (which skipped ticks and slammed the throttle on/off). The integrator only runs while the rate request is unsaturated, so a 400-block climb no longer arrives with a wound-up integrator and a +5 overshoot.
