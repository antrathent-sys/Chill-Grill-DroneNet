#!/usr/bin/env python3
"""Run the control room tests (lib/state.lua, lib/screens.lua) on Lua 5.1.
    python tools/run_screens_test.py"""
import os, sys
...
try:
    entry(os.path.join(HERE, "test_screens.lua").replace("\\", "/").encode(), HERE.replace("\\", "/").encode())
