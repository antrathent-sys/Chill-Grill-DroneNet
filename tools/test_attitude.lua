...
local function simulate(q, mounts)
  ...
  -- invert gravityFromGimbal to get the gimbal angles this attitude implies
  local pitch = math.deg(math.asin(math.max(-1, math.min(1, gBody.z))))
  local roll  = math.deg(math.atan2(-gBody.x, -gBody.y))
  local tables = {}
